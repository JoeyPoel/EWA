...
-->
  <v-select
      v-if="filterItems && filterItems.items.length > 0"
      clearable="true"
      chips="true"
      :items="filterItems.items"
      item-title="name"
      item-value="id"
      label="Filter by {{ filterItems.label }}"
      ...
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template #activator="{ props }">
            <v-btn class="mb-2" color="primary" dark v-bind="props">New {{ modelName }}</v-btn>
          </template>
          <v-card>
            <v-card-title>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.stateList[dialogState].title;
    },
  },
